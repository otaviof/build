...
func validateController(
	...
	// ensure build strategy is present in the same namespace
	buildStrategyName := testBuild.Spec.StrategyRef.Name
	buildStrategyNsName := types.NamespacedName{Namespace: ns, Name: buildStrategyName}
	t.Logf("Waiting for build-strategy '%s'...", buildStrategyName)
	require.Eventually(t, func() bool {
		buildStrategy := &operator.ClusterBuildStrategy{}
		err := f.Client.Get(goctx.TODO(), buildStrategyNsName, buildStrategy)
		return err == nil
	}, 60*time.Second, 1*time.Second, "BuildStrategy is not found")
	...
	}, 300*time.Second, 5*time.Second, "BuildRun is pending or running")
	...
	}, 550*time.Second, 7*time.Second, "BuildRun not succeeded")
